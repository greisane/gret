...
UVPoint = namedtuple('UVPoint', ['u', 'v', 'vert_index'])  # Used to uniquely identify 'welded' loops
UVPoint.__repr__ = lambda self: f"UVPoint(i={self.vert_index}, u={self.u:.3f}, v={self.v:.3f})"
...
def get_selection_bags(bm):
    ...
    for face in bm.faces:
        ...
        for loop in face.loops:
            ...
            if not uv.select:
                continue
            point = UVPoint(*uv.uv, loop.vert.index)
            ...
            for other_loop in (loop.link_loop_next, loop.link_loop_prev):
                ...
                if other_uv.select:
                    other_point = UVPoint(*other_uv.uv, other_loop.vert.index)
